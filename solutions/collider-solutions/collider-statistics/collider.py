...
class ChemicalSphere(Sphere):
    ...
    def check_sphere_collision(self, other):
        ...
        dy = self.y - other.y
        distance = np.sqrt(dx**2 + dy**2)
        if distance < self.radius + other.radius and self.atom == nitrogen:
...
class Simulation:
    ...
    def run(self, fig=None):
        ...
        while True:
            ...
            res['avg_speeds'].append(np.mean([np.sqrt(sp.vx**2 + sp.vy**2) for sp in self.spheres]))
            step +=1 

            if fig is not None:
                fig.clear()
                ax = fig.add_subplot(211)
                ax.plot(list(range(step)), res['avg_speeds'], "o", markersize=3)
                ax.set_ylabel('Average speed')

                ax = fig.add_subplot(212)

                for molecule in [oxygen, nitrogen, nitric_oxide]:
                    res[f'{molecule.name}_counts'].append(sum([1 for sp in self.spheres if sp.atom == molecule]))
                    ax.plot(range(step), res[f'{molecule.name}_counts'], "o", color=molecule.color.fill, markersize=3)
