    ...
    def check_collision(self, sphere):
        ...
        if self.x1 == self.x2:
            if self.x1 - sphere.radius < sphere.x < self.x1 + sphere.radius:
                if (
                    self.y1 - sphere.radius < sphere.y < self.y2 + sphere.radius
                    or self.y2 - sphere.radius < sphere.y < self.y1 + sphere.radius
                ):
        ...
        # Check for horizontal walls
            ...
            if self.y1 - sphere.radius < sphere.y < self.y1 + sphere.radius:
                if (
                    self.x1 - sphere.radius < sphere.x < self.x2 + sphere.radius
                    or self.x2 - sphere.radius < sphere.x < self.x1 + sphere.radius
                ):
...
class Simulation:
    ...
    def __init__(self, canvas, width, height, num_spheres, maxwell_box=False, temperature=100):
        self.width = width
        self.height = height
        ...
        if maxwell_box:
            self.spheres, self.walls = self.create_maxwell_box(num_spheres)
        else:
    ...
    def create_maxwell_box(self, num_spheres):
        walls = self.create_side_walls()
        thickness = 5
        # middle walls
        walls.append(Wall(self.width / 2 - thickness, 0, self.width / 2 - thickness, 3 * self.height / 8))
        walls.append(Wall(self.width / 2 - thickness, self.height, self.width / 2 - thickness, 5 * self.height / 8))
        walls.append(Wall(self.width / 2 + thickness, 0, self.width / 2 + thickness, 3 * self.height / 8))
        walls.append(Wall(self.width / 2 + thickness, self.height, self.width / 2 + thickness, 5 * self.height / 8))

        walls.append(Wall(self.width / 2 - thickness, 3 * self.height / 8, self.width / 2 + thickness, 3 * self.height / 8))
        walls.append(Wall(self.width / 2 - thickness, 5 * self.height / 8, self.width / 2 + thickness, 5 * self.height / 8))

        spheres = []

        def create_sphere(left, right, bottom, top, atom):
            x = np.random.randint(left, right)
            y = np.random.randint(bottom, top)
            vx = np.sqrt(self.temperature) / 20 * np.random.uniform(-0.5, 0.5)
            vy = np.sqrt(self.temperature) / 20 * np.random.uniform(-0.5, 0.5)
            return ChemicalSphere(x, y, vx, vy, atom)

        for _ in range(int(num_spheres/2)):
            margin = 20
            sp = create_sphere(margin, self.width / 2 - margin, margin, self.height - margin, oxygen)
            spheres.append(sp)
            sp = create_sphere(
                self.width / 2 + margin, self.width - margin, margin, self.height - margin, nitrogen
            )
            spheres.append(sp)
